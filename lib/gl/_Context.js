...
function equalState(stateA,stateB){
    return stateA[0] == stateB[0];
}

function lastState(state){
    return state[1];
}
...
function Context(element,canvas3d,canvas2d){
    ...
    var program;
    program = this._program = new Program(this, Shader.vert,     Shader.frag, attributesToBind);
    this._uniformLocationMatrix     = program[ShaderDict.uMatrix];
    this._uniformLocationViewport   = program[ShaderDict.uViewport];
    this._uniformLocationFlipY      = program[ShaderDict.uFlipY];
    this._uniformLocationImage      = program[ShaderDict.uImage];
    this._uniformLocationUseTexture = program[ShaderDict.uUseTexture];
    this._attribLocationPosition    = program[ShaderDict.aVertPosition];
    this._attribLocationColor       = program[ShaderDict.aVertColor];
    this._attribLocationTexcoord    = program[ShaderDict.aTexCoord];
    ...
    this._colorBg       = new Float32Array(4);
    ...
    this._modePolylineCap = Context.ROUND;
    ...
    model = this._circle = new Model(gl);
    model.addAttribute('vertex', new Float32Array(bVertexEllipseLen));
    model.addAttribute('vertexS',new Float32Array(bVertexEllipseLen));
    model.addAttribute('vertexT',new Float32Array(bVertexEllipseLen));
    model.addAttribute('color',  new Float32Array(ELLIPSE_DETAIL_MAX * 4));
    ...
    this._vboCircle         = new Vbo(gl.ARRAY_BUFFER,bVertexEllipseLen * 3 * 4 + 4 * ELLIPSE_DETAIL_MAX * 4,gl.DYNAMIC_DRAW);
    this._vboCircleData     = {};
    this._iboCircle         = new Vbo(gl.ELEMENT_ARRAY_BUFFER);
    this._iboCircleData     = {};
    ...
    this._fill        = true;
    this._colorFill4 = [1.0,1.0,1.0,1.0];
    this._colorFill  = this._colorFill4;

    this._stroke             = true;
    this._colorStroke4      = [1.0,1.0,1.0,1.0];
    this._colorStroke       = this._colorStroke4;
...
Context.prototype._setSize = function(width, height){
    ...
    this._fboCanvas.size(iwidth,iheight);
    this._fboPingpong.size(iwidth,iheight);
...
Context.prototype._resetDrawProperties = function(){
    ...
    this.circleMode(Context.CENTER);
...
Context.prototype.fill = function(){
    var f = this._colorFill = this._colorFill4;
    f[3] = 1.0;
...
Context.prototype.fill1f = function(k){
    var bColor = this._colorFill = this._colorFill4;
...
Context.prototype._stroke4f = function(r,g,b,a){
    var bColor = this._colorStroke4Temp;
        bColor[0] = r;
        bColor[1] = g;
        bColor[2] = b;
        bColor[3] = a;
...
Context.prototype._drawFbo = function(fbo,x,y,width,height){
    ...
    width  = typeof width === 'undefined' ? fbo.width() : width;
    height = typeof height=== 'undefined' ? fbo.height(): height;
...
Context.prototype.background = function(){
    ...
    this._stackColorBg.push(this._colorTemp);
};
...
Context.prototype._quad_internal = function(x0,y0,x1,y1,x2,y2,x3,y3){
    ...
    if(this._fill && !this._texture){
        bColor = this.bufferColors(this._colorFill,this._bColorQuad_internal);
...
Context.prototype.roundRect = function(x,y,width,height,radius){
    ...
    if(this._fill && !this._texture){
        colors = this.bufferColors(this._colorFill4,this._bColorRRect);
        if(this._batchActive){}
        else{
...
Context.prototype.ellipse = function(x,y,radius_x,radius_y){
    ...
    if(this._fill && !this._texture){
        colors = this.bufferColors(this._colorFill,this._bColorEllipse);
...
Context.prototype._setPointerPosition = function(offset){
    this._program.vertexAttribPointer('aVertexPosition',2,this._gl.FLOAT,false,0,offset);
};

Context.prototype._setPointerColor = function(offset){
    this._program.vertexAttribPointer('aVertexColor',4,this._gl.FLOAT,false,0,offset);
};

Context.prototype._setPointerTexcoord = function(offset){
    this._program.vertexAttribPointer('aTexcoord',2,this._gl.FLOAT,false,0,offset);
};

Context.prototype._enableArrayColor = function(bool){
    if(bool){
        this._program.enableVertexAttribArray('aVertexColor');
        return;
    }
    this._program.disableVertexAttribArray('aVertexColor');
};

Context.prototype._enableArrayTexcoord = function(bool){
    if(bool){
        this._program.enableVertexAttribArray('aTexcoord');
        return;
    }
    this._program.disableVertexAttribArray('aTexcoord');
};


Context.prototype.circle = function(x,y,radius){
    if(!this._fill && !this._stroke && !this._texture){
        return;
    }
    var gl = this._context3d;
    ...
    if(this._fill && !this._texture){
        colors = this.bufferColors(this._colorFill,this._bColorCircle);
...
Context.prototype.arc = function(x, y, radius_x, radius_y, angle_start, angle_stop, radius_x_inner, radius_y_inner){
    ...
    if(this._fill && !this._texture){
        var colors = this.bufferColors(this._colorFill,this._bColorArc);
...
Context.prototype.triangle = function(x0,y0,x1,y1,x2,y2){
    ...
    if(this._fill && this._texture)
    {
        c = this.bufferColors(this._colorFill,this._bColorTriangle);
...
Context.prototype.point = function(x,y)
{
    ...
    var v  = this._bVertexPoint,
        c  = this.bufferColors(this._colorFill4,this._bColorPoint);
...
Context.prototype.pointSet = function(vertexArrOrFloat32Arr){
    ...
    this.bufferArrays(Util.safeFloat32Array(vertexArrOrFloat32Arr),
        this.bufferColors(this._colorFill,new Float32Array(vertexArrOrFloat32Arr.length*2)));
...
Context.prototype._polyline = function(points,length,loop){
    ...
    if( color_stoke_length != 4){
        if(color_stoke_length != length * 2){
            var b_color_intrpl = Color.colorvLerped(color_stroke,Util.arrayResized(point_num * 4, this._colorStrokeIntrpl));
...
Context.prototype.circleSet = function(posArr,radiusArr,fillColorArr,strokeColorArr,texcoordArr){
    ...
    var bColorFill4 = this._colorFill4;
    ...
    if(!this._texture){
        ...
        if(!fillColorArr){
            var colorFill4f = this._colorFill4;
...
Context.prototype.drawArrays = function(verticesArrOrFloat32Arr, colorArrOrFloat32Arr, mode){
    ...
    var vertices = Util.safeFloat32Array(verticesArrOrFloat32Arr),
        colors   = this.bufferColors((colorArrOrFloat32Arr || this._colorFill4),
...
Context.prototype.drawElements = function(vertices,indices,colors,mode,length){
    ...
    colors = colors ? (colors.length == colorsExpLength ?
        ...
        this.bufferColors(this._colorFill4, new Float32Array(colorsExpLength));
    ...
    if(colors){
        ...
        colors = this.bufferColors(this._colorFill4, new Float32Array(colorsExpLength));
    }
