...
function Context(element,canvas3d,canvas2d){
...

    {
        ...
        this._uColor = new Float32Array([0,0,0,1.0]);
        ...
        this._stateColorStroke  = state1Create();
    ...
    function createPrimitiveModel(numPoints){
        ...
        var model = {};
        //vertices caps & edges
        model.vertex = new Float32Array(numPoints * 2);
        //colors caps & edges
        model.color = new Float32Array(numPoints * 4);
        //vertex buffer vertices & colors caps & edges
        model.vbo = new Vbo(gl.ARRAY_BUFFER,model.vertex.byteLength + model.color.byteLength,gl.DYNAMIC_DRAW);
        return model;
    }
    ...
    //Point
    this._point = createPrimitiveModel(1);
    this._point.stateColor  = state4Create();
    this._point.stateOrigin = state2Create();

    //Point set
    this._points = createPrimitiveModel();
    this._points.stateColor = stateArrCreate();
    ...
    // Circle
    {
        numPoints = Default.ELLIPSE_DETAIL_MAX; bufferDataLength = 0;
...
Context.prototype.background = function(){
    var color  = this._colorTemp;
    color[3] = 1.0;

    switch (arguments.length){
        case 0: color[0]=color[1]=color[2]=0.0;break;
        case 1: color[0]=color[1]=color[2]=arguments[0];break;
        case 2: color[0]=color[1]=color[2]=arguments[0];color[3]=arguments[1];break;
        case 3: color[0]=arguments[0];color[1]=arguments[1];color[2]=arguments[2];break;
        case 4: color[0]=arguments[0];color[1]=arguments[1];color[2]=arguments[2];color[3]=arguments[3];break;
    }

    this._clearBackground = (color[3] == 1.0);
    state4Push(this._stateColorBg,this._colorTemp);
};
...
Context.prototype._enablePerVertexColor = function(bool){
    ...
    program.uniform4fv('uColor',this._uColor);
};
...
Context.prototype.circleDetail = function(a){
    ...
    state1Push(state,Math.min(a,Default.ELLIPSE_DETAIL_MAX));
};
        ...
        case 1:
            ...
            if(isArray(color)){
                ...
                if(colorLen > fill.length){
                    fill = this._colorFill = new Float32Array(colorLen);
                }
                fill.set(color);
...
Context.prototype.stroke = function(){
    ...
    switch (arguments.length){
        ...
        case 1:
            ...
            if(isArray(color)){
                ...
                if(colorLen > stroke.length){
                    stroke = this._colorStroke = new Float32Array(colorLen);
                }
                stroke.set(color);
            ...
            var state  = this._stateColorStroke;
            state1Push(this._stateColorStroke,Util.copyArray(color));
        ...
        case 2:
            var k =  arguments[0];
            this._stroke4f(k,k,k,stroke[3]);
            break;
        case 3:
            this._stroke4f(stroke[0],stroke[1],stroke[2],1.0);
            break;
        case 4:
            this._stroke4f(stroke[0],stroke[1],stroke[2],stroke[3]);
            break;
    }
    this._stroke = true;
    color = this._uColor;
...
Context.prototype._stroke4f = function(r,g,b,a){
    var color = this._colorStroke4Temp;
    color[0] = r;
    color[1] = g;
    color[2] = b;
    color[3] = a;
    var state = this._stateColorStroke;
    state1Push(state,Util.copyArray(color));
