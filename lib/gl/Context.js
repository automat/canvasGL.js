...
var UNSIGNED_DATA_TYPE,
    UNSIGNED_DATA_BYTES;
//Float32Array || Float64Array on use64Bit
var FloatArray;
//4
var FLOAT_DATA_BYTES;
...
function Context(element,canvas3d,canvas2d){
...

    {
        ...
        this._colorStroke4Temp  = new Float32Array(4);
...
Context.prototype._polyline = function(model,points,length,loop){
    ...
    var stateColor   = model.stateColor,
        color        = stateArrFront(stateColor),
...
Context.prototype.stroke = function(){
    var stroke = this._colorStroke;
    var lineColorState  = this._line.stateColor,
        linesColorState = this._lines.stateColor,
        linepColorState = this._linep.stateColor;
        ...
        case 1: //k,1 || arr
            if(isArray(arguments[0])){
                ...
                if(len > stroke.length){
                    stroke = this._colorStroke = new Float32Array(len);
                }
                stroke.set(color);

                state1Push(lineColorState, stroke);
                state1Push(linesColorState,stroke);
                state1Push(linepColorState,stroke);
            } else {
                r = g = b = arguments[0];
                a = 1.0;
            }
    ...
    //update all line model color states, with single color
    if(len == 4){
        stroke[0] = r;
        stroke[1] = g;
        stroke[2] = b;
