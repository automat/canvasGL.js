...
function Context(element,canvas3d,canvas2d){
    ...
    //Polyline
    {
        ...
        this._statePointsLine   = stateArrCreate();
        this._statePointsLength = state1Create();

        //all line components edges + caps
        this._lineComponents = {
            vertex : {
                //Round line cap unit
                cap  : GeomUtil.genVerticesCircle(numPoints / 2, new Float32Array(numPoints)),
                //Round line cap scaled
                capS : new Float32Array(numPoints),
                //Round line cap translated
                capT : new Float32Array(numPoints),
                //rectangle edge of the line
                edge : new Float32Array(4 * 2)
            },
            index  : {
                //indices for cap scheme
                cap : ModelUtil.genFaceIndicesFan(numPoints),
                //indices for cap translated
                capT : new Array((numPoints / 2 - 2) * 3),
                //indices for edge scheme
                edge  : new UintArray([0,1,2,1,2,3]),
                //indices for edge translated
                edgeT : new UintArray(6)
            }
        };
...
Context.prototype.lines = function(points,colors){};
...
Context.prototype._polyline = function(points,length,loop){
    var stateWeight = this._stateWeightStroke,
        weight      = state1Front(stateWeight);

    if(!this._stroke || weight == 0.0){
        return;
    }

    var weight_2 = weight * 0.5;

    var stateColor  = this._stateColorStroke,
    ...
    var component       = this._lineComponents,
    ...
    var statePoints       = this._statePointsLine,
        statePointsLength = this._statePointsLength;

    //if the stroke width changed, rescale the cap scheme accordingly
    if(!state1Equal(stateWeight)){
        VertexUtil.scale(componentVertex.cap,weight_2,weight_2,componentVertex.capS);
    }

    // cap data
    var vertexCap  = componentVertex.capS, // the base cap, scaled to linewidth
        vertexCapT = componentVertex.capT; // container for translated cap vertices
    var indexCap   = componentIndex.cap,   // unit cap index scheme
        indexCapT  = componentIndex.capT;  // container for translated cap indices
    var lenVertexCap = vertexCap.length,
        numVertexCap = lenVertexCap * 0.5;
    var lenIndexCap  = indexCap.length;

    // edge data
    var indexEdge  = componentIndex.edge,
        indexEdgeT = componentIndex.edgeT;
    ...
    var numVertexBuffer = numVertexOffsetEdges + numVertexEdge * numEdges,
        lenVertexBuffer = numVertexBuffer * 2;

    // index data
    var lenIndexOffsetEdges = lenIndexCap * numPoints;
    var lenIndexBuffer = lenIndexOffsetEdges + lenIndexEdge * numEdges;
    ...
    var vertexEdge = new Float32Array(8);
    var vertexData = new Float32Array(lenVertexBuffer);
    ...
    while(++i < numPoints){
        ...
        //translated the scaled cap to the new position
    ...
    gl.bufferData(gl.ARRAY_BUFFER,vertexData.byteLength ,gl.STATIC_DRAW);
    ...
    var indexData = new UintArray(lenIndexBuffer);
    ...
    this.applyMatrixUniform();
    //gl.drawArrays(gl.POINTS,0,numVertexBuffer);
    //gl.drawArrays(gl.POINT_SIZE,numVertexOffsetEdges,numVertexBuffer-numVertexOffsetEdges);

    //gl.drawElements(gl.TRIANGLES,lenIndexBuffer,UNSIGNED_DATA_TYPE,0);
    //gl.drawElements(gl.TRIANGLES,3, UNSIGNED_DATA_TYPE, 3 * UNSIGNED_DATA_BYTES);
    ...
    //Update color & point state
    stateArrPush(stateColor, Util.copyArray(color));
    stateArrPush(statePoints, Util.copyArray(points));
    //Update length & width
...
Context.prototype._stroke4f = function(r,g,b,a){
    ...
    bColor[2] = b;
    bColor[3] = a;
    state1Push(this._stackColorStroke,Util.copyArray(bColor));
    this._stroke = true;
};
