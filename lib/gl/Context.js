...
function Context(element,canvas3d,canvas2d){
    ...
    //Polyline
    {
        ...
        function createPolylineModel(){
            ...
            //offsets
            model.numOffsetEdges  = 0;
            model.numOffsetColors = 0;
            model.lenOffsetEdgesIndex = 0;
            //vertices caps & edges
...
Context.prototype.lines = function(points,colors,loop){
    ...
    if(typeof first === 'object' &&
       ...
       typeof first.splice === 'function'){
        ...
        while(++i < l){
            line = points[i];
            this._polyline(model,line,colors,line.length,loop);
        }
    } else {
        this._polyline(model,points,colors,points.length,loop);
    }
};
...
Context.prototype._polyline = function(model,points,colors,length,loop){
    ...
    //num caps + edges + colors
    var numVertexCaps   = numVertexCap  * numPoints,
        numVertexEdges  = numVertexEdge * numEdges,
        numVertexColors = numVertexCaps + numVertexEdges;

    //offsets
    var numOffsetEdges  = numVertexCaps;

    var numVertexData = numVertexCaps + numVertexEdges,
        numColorData  = numVertexColors;
    var lenVertexData = numVertexData * 2,
        lenColorData  = numColorData * 4;

    var numVertexBuffer = numVertexData + numColorData,
        lenVertexBuffer = numVertexBuffer * 2;

    //index data length
    var lenIndexOffsetEdges, lenIndexBuffer;
    ...
    if(lengthExceeded){
        lenIndexOffsetEdges = lenIndexCap * numPoints;
        ...
        indexData  = model.index  = new UintArray(lenIndexBuffer);

        model.numOffsetEdges  = numVertexCap * numPoints;
        model.numOffsetColors = numOffsetEdges;

        model.lenOffsetEdgesIndex = lenIndexOffsetEdges;
    ...
    lenIndexOffsetEdges = model.lenOffsetEdgesIndex;
    lenIndexBuffer      = indexData.length;

    numOffsetEdges  = model.numOffsetEdges;

    var lenOffsetVertexEdges  = numOffsetEdges * 2;
    ...
    while(++i < numPoints){
        ...
        if(i < numEdges){
            ...
            vertexData.set(vertexEdge,lenOffsetVertexEdges + i * lenVertexEdge);
        }
    }
    ...
    if(detailDiffers || lengthExceeded){
        ...
        while(++i < numPoints){
            ...
            if(i < numEdges){
                j = numOffsetEdges + i * numVertexEdge;
